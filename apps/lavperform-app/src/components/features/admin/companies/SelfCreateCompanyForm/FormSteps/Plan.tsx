...
function PlanComponent(props: FormStepsProps) {
  ...
  const filteredPlans = useMemo(() => {
    ...
    if (props.planIdFromUrl) {
      return plans.filter((plan) => plan.id === props.planIdFromUrl)
    }

    return plans
  }, [plans, props.planIdFromUrl])
  ...
  const {
    ...
    values: {
      planId: props.formData?.planId || props.planIdFromUrl || '',
    },
  })
  ...
  if (props.planIdFromUrl && filteredPlans.length === 0) {
    return (
      <Empty
        description="O plano informado na URL não está disponível. Verifique o link ou entre em contato com o suporte."
