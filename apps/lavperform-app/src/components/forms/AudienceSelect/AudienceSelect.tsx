...
export function AudienceSelect({
  ...
  const { data, isLoading } = useAudiences(selectedCompany?.id, {
    page: 1,
    limit: 100,
  })
  ...
  const audiences = useMemo<Audience[]>(() => data?.data ?? [], [data])

  const filteredAudiences = useMemo(() => {
    const q = normalizeSearch(searchQuery)
    if (!q) return audiences
    return audiences.filter((audience) =>
      normalizeSearch(audience.name).includes(q)
    )
  ...
  return (
    <Field.Root invalid={showError}>
      ...
      <Controller
        ...
        render={({ field }) => (
          ...
          >
            ...
            <Select.Control>
              <Select.Trigger>
                <Select.ValueText placeholder="Selecione uma audiência" />
              </Select.Trigger>
              ...
              <Select.Content maxH={320}>
                ...
                {filteredAudiences.length === 0 ? (
                  <Box color="fg.muted" pb={3} pt={1} px={3} textAlign="center">
                    <Text fontSize="sm">Nenhuma audiência encontrada</Text>
                  </Box>
                ) : (
                  collection.items.map((item) => (
                    ...
                    >
                      {item.name} (Clientes:{' '}
                      {(item.customerCount ?? 0).toLocaleString('pt-BR')})
      ...
      />
      <Field.HelperText>
        <RouterLink to="/customers/audiences">Criar ou gerenciar audiências</RouterLink>
      </Field.HelperText>
