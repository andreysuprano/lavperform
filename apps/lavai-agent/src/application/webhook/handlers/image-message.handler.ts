...
import type { ImageIncomingMessage, IncomingMessage } from '../types/incoming-message.types';
...
@Injectable()
export class ImageMessageHandler implements MessageHandlerPort {
  private readonly logger = new Logger(ImageMessageHandler.name);
  ...
  async handle(message: IncomingMessage, agent: AgentWithConfigsData): Promise<void> {
    const msg = message as ImageIncomingMessage;
    ...
    if (!accessResult.allowed) {
      this.logger.debug(`[IMAGE] Acesso bloqueado: ${accessResult.reason}`);
      return;
    }
    ...
    if (!triggerResult.allowed) {
      this.logger.debug(`[IMAGE] Gatilho não acionado na legenda: ${triggerResult.reason}`);
      return;
    }

    // 3. Normalização completa (Vision) — só é chamada após passar pelos filtros
    this.logger.log(`[IMAGE] Normalizando imagem (Vision) para agent=${agent.id}...`);
    const prompt = await this.normalizer.normalize(message, agent);

    this.logger.log(
      `[IMAGE] agent=${agent.id} | sender=${prompt.context.senderPhone} | caption="${captionText.slice(0, 60)}"`,
    );
    ...
    this.logger.log(`[IMAGE] conversation=${conversation.id} | invocando LLM`);
