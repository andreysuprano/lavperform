import { postLaundrykitRoute, LaundryKitRouteArgs } from '../clients/laundrykit.client'
import { LaundryKitOperation } from './laundrykit-to-ingest'
...
function pickName(details?: LaundryKitClientDetails, fallback?: string): string {
  return (
    details?.Name?.trim() ||
    details?.name?.trim() ||
    fallback?.trim() ||
...
export class LaundryKitClientCatalog {
  ...
  static fromClients(clients: LaundryKitStoreClient[]): LaundryKitClientCatalog {
    ...
    for (const client of clients) {
      ...
      const details = pickDetails(user)
      const userIds = [client.USER_ID, user?.USER_ID, user?.user_ID]
        .map(normalizeUserId)
  ...
  lookup(operation: LaundryKitOperation): LaundryKitStoreClient | undefined {
    const userIds = [operation.USER_ID, operation.USER?.ID]
      .map(normalizeUserId)
      .filter(Boolean) as string[]
    ...
    const identifiers = [operation.USER_IDENTIFIER, operation.USER?.IDENTIFIER]
      .map(digitsOnly)
      .filter(Boolean) as string[]
