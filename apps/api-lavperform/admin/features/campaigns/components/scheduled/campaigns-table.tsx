...
import { enrichPaginationMeta, formatDate, getPrimaryMetric } from "../../utils"
...
export function CampaignsTable({
  ...
  onChangeStatus: (campaign: Campaign, status: Campaign["status"]) => void
  onReprocess: (campaign: Campaign) => void
  ...
  const columns = React.useMemo<ColumnDef<Campaign>[]>(
        ...
        cell: ({ row }) => (
          <div className="flex justify-end">
            <CampaignActionsMenu
              ...
              onChangeStatus={(status) => onChangeStatus(row.original, status)}
    ...
    [onChangeStatus, onReprocess, onDelete]
  )
        ...
        <div className="flex flex-col items-center justify-between gap-3 px-1 md:flex-row">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <label htmlFor="campaign-page-size" className="text-xs text-muted-foreground">
                Itens por página
              </label>
              <select
